...
exports.handler = async function (event) {
    ...
    try {
        const { username, id_expediente, nuevo_cliente } = JSON.parse(event.body);
        ...
        // Lo convierte en padre (id_padre = NULL) y le asigna el nuevo cliente
        await client.query(`UPDATE expedientes SET cliente = $1, id_padre = NULL WHERE id_expediente = $2 AND username = $3`, [nuevo_cliente, id_expediente, username]);
        await client.end();
        return { statusCode: 200, body: JSON.stringify({ success: true }) };
    } catch (error) { return { statusCode: 500, body: JSON.stringify({ error: 'Fallo al reclasificar cliente.' }) }; }
};